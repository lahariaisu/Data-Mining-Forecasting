/* Adapted from Project/Log_Transformation.sas
   ...
   (Dec 2004 - Nov 2006) so the same log-transform logic runs standalone --
   the runner ships only script text, no data files, so the sample travels
   with the script as datalines. */
...
/* --- unmodified from Log_Transformation.sas below --- */
data work.forecasting_finalproject;
