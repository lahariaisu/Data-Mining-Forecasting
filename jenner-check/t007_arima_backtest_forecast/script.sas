/* Adapted from Project/STSM03s04(residual).sas, first block (labeled
   "STSM03s04a.sas" in a comment inside the file -- the univariate backtest
   stage of the author's model-validation pipeline, before the file moves on
   to crosscorr/prewhitening stages and %include's an external macro file
   not present in this repo). Original reads STSM.FORECASTING_FINALPROJECT;
   substituted here with the same 60-row inline slice of the repo's own
   Project/Rainfall_data.csv used in t004 (Jan 2000 - Dec 2004) and WORK in
   place of STSM. The IDENTIFY/ESTIMATE/FORECAST statements -- including the
   BACK=12 backtest window, distinct from the pure-forecast bundles
   elsewhere in this set -- are byte-for-byte unchanged. */
...
/* --- unmodified from STSM03s04(residual).sas, first block (STSM -> WORK) --- */
ods graphics off;

proc arima data=work.forecasting_finalproject;
...
proc print data=work.out(obs=15);
run;
