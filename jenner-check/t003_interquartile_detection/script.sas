/* Adapted from Project/InterQuartile_Detection.sas
   Original reads stsm.forecasting_finalproject (a local SAS library pointed
   at the author's forecasting_finalproject.sas7bdat). Substituted here with
   a 24-row inline slice of the repo's own Project/Rainfall_data.csv
   (Dec 2004 - Nov 2006) so the same PROC UNIVARIATE call runs standalone. */
...
/* --- unmodified from InterQuartile_Detection.sas below --- */
proc univariate data=work.forecasting_finalproject;
  var Precipitation;
